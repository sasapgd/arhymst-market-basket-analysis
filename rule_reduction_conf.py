"""Reduce association rules using confidence improvement.

Rules are compared only when they have the same consequent. An expanded rule
is removed when an already retained subset of its antecedent improves
confidence by less than ``delta``. The manuscript baseline uses delta = 0.05.
"""
...
DEFAULT_DELTA = 0.05
REQUIRED_COLUMNS = ("Premises", "Conclusion", "Confidence")
NUMERIC_COLUMNS = ("Support", "Confidence", "Lift")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Remove rules with insufficient confidence improvement."
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        help="Reduced-rules CSV (default: INPUT stem plus _REDUCED.csv).",
    )
    parser.add_argument(
        "--delta",
        type=float,
        default=DEFAULT_DELTA,
        help="Minimum confidence improvement (default: 0.05).",
    )
...
def resolve_paths(args: argparse.Namespace) -> tuple[Path, Path, Path, Path]:
    ...
    output_file = (
        ...
        if args.output
        else input_file.with_name(f"{input_file.stem}_REDUCED.csv")
    )
    output_file.parent.mkdir(parents=True, exist_ok=True)

    timing_file = output_file.parent / "RULE_REDUCTION_TIMING_SUMMARY.csv"
    metadata_file = output_file.parent / "RULE_REDUCTION_RUN_METADATA.csv"
...
def process_consequent_group(rules: list[dict], delta: float) -> set[object]:
    ...
    # rules against which confidence improvement of expanded antecedents is tested.
    ...
    for rule in rules:
        ...
        for subset_size in range(1, rule["antecedent_size"]):
            ...
            for subset in combinations(rule["antecedent"], subset_size):
                parent_confidence = retained_subsets.get(subset)
        ...
        # confidence as the reference for rules with larger antecedents.
        same_size = retained_by_size[rule["antecedent_size"]]
        previous_best = same_size.get(rule["antecedent"])
        if previous_best is None or rule["confidence"] > previous_best:
            same_size[rule["antecedent"]] = rule["confidence"]

    return removed


def reduce_rules(frame: pl.DataFrame, delta: float) -> tuple[pl.DataFrame, int]:
    groups: dict[tuple[str, ...], list[dict]] = defaultdict(list)

    for rule_id, premises, conclusion, confidence in frame.select(
...
def run(args: argparse.Namespace) -> None:
    ...
    step_start = perf_counter()
    reduced, consequent_groups = reduce_rules(prepared, args.delta)
    ...
    print("Confidence-improvement reduction completed.")
